...
<%
  ...
  try{
    ...
    int try_num = list.size();

    if (list.size() > 0) {
        hit_flag = 1;
        for (int i = 0; i < try_num; i++) {
            SQL = new StringBuffer();
            SQL.append("select article_no,title from article_tbl where article_no = '");
            SQL.append(list.get(i).get("article_no"));
            SQL.append("'");
            rs = stmt.executeQuery(SQL.toString());
            while (rs.next()) {
                map = new HashMap<String, String>();
  ...
  <body>
    <div class="container-fluid bg-slider">
      <div class="col-2 pt-3 position-fixed">
        ...
        <a href="../rank/rank.jsp?genre=1"><div class="col-8 text-center menu_item"><i class="fas fa-award logo"></i><div class="menu_name">RANKING</div></div></a>
        ...
        <div class="offset-2 my-4">
            <div class="main col-10 offset-1">
                <% if(hit_flag == 1){ %>
                  <% for(int i = list.size() - 1; list.size()/2 <= i; i--){ %>
                ...
                <% }else{ %>
                  <div class="alert alert-success" role="alert">
                    <h4 class="alert-heading">お気に入り記事が存在しません</h4>
                    お気に入りしてね
                  </div>
                <% } %>
            </div>
